...
def find_max_violation(a, b, rtol=None, atol=None):  # pylint: disable=C0103
...
def assert_almost_equal(a, b, rtol=None, atol=None, names=('a', 'b'), equal_nan=False):
    ...
    msg = np.testing.build_err_msg(
        [a, b],
        err_msg=f'Error {rel} exceeds tolerance rtol={rtol}, atol={atol}. ' +
                f'Location of maximum error:{str(index)}, a={a[index]}, b={b[index]}',
        names=names)
