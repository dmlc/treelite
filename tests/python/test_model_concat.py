...
def test_model_concat_with_tree_stump():
    ...
    def make_tree_stump():
        ...
        tree = treelite.ModelBuilder.Tree(threshold_type="float32",
                                          leaf_output_type="float32")
        tree[0].set_numerical_test_node(
            feature_id=0, opname='<', threshold=0.0, default_left=True,
            left_child_key=1, right_child_key=2, threshold_type="float32")
...
def test_model_concat_with_mismatched_tree_types():
    ...
    def make_tree_stump(*, threshold_type, leaf_output_type):
        ...
        tree = treelite.ModelBuilder.Tree(threshold_type=threshold_type,
                                          leaf_output_type=leaf_output_type)
        tree[0].set_numerical_test_node(
            feature_id=0, opname='<', threshold=0.0, default_left=True,
            left_child_key=1, right_child_key=2, threshold_type=threshold_type)
