 ...
 * \file lightgbm.cc
 * \brief Parser for lightgbm model
 * \author Philip Cho
 */
...
class LGBParser : public Parser {
 public:
  LGBParser() { LOG(INFO) << "LGBParser yah"; }
  ...
  Model Export() const override {
    ...
    for (const auto& lgb_tree : lgb_trees_) {
      ...
      while (!Q.empty()) {
        ...
        if (old_id < 0) {  // leaf
          tree[new_id].set_leaf(static_cast<tl_float>(lgb_tree.leaf_value[~old_id]));
        } else {  // non-leaf
          const unsigned split_index = static_cast<unsigned>(lgb_tree.split_feature[old_id]);
          const tl_float threshold = static_cast<tl_float>(lgb_tree.threshold[old_id]);
          const tl_float default_value = static_cast<tl_float>(lgb_tree.default_value[old_id]);
          bool default_left;
          Tree::Operator cmp_op;
          switch (lgb_tree.decision_type[old_id]) {
            case DecisionType::numerical:
              cmp_op = Tree::Operator::kLE;
              default_left = (default_value <= threshold);
              break;
            case DecisionType::categorical:
              cmp_op = Tree::Operator::kEQ;
              default_left = (default_value == threshold);
              break;
            default:
...
TREELITE_REGISTER_PARSER(LGBParser, "lightgbm")
.describe("Parser for lightgbm format")
.set_body([]() {
    return new LGBParser();
  });
}  // namespace parser
}  // namespace treelite
