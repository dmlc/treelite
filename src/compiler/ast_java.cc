...
class ASTJavaCompiler : public Compiler {
  ...
  CompiledModel Compile(const Model& model) override {
    ...
    files_[file_prefix_ + "PredictorJavaWrapper.java"]
      = fmt::format(java::predictor_java_wrapper_template,
          "java_package"_a = param.java_package,
          "pred_logic"_a = (num_output_group_ > 1 ? java::pred_logic_multiclass
                                                  : java::pred_logic));
    files_["src/main/java/ml/dmlc/treelite4j/Data.java"]
      = java::data_interface;
    files_["src/main/java/ml/dmlc/treelite4j/InferenceEngine.java"]
      = java::inference_engine_interface;
    files_["pom.xml"]
 ...
 private:
  ...
  void HandleMainNode(const MainNode* node,
    ...
    AppendToBuffer(dest,
      fmt::format(java::main_start_template,
        "java_package"_a = param.java_package,
    ...
    if (num_output_group_ > 1) {
      AppendToBuffer(dest,
        fmt::format(java::main_end_multiclass_template,
          "num_output_group"_a = num_output_group_,
      ...
      AppendToBuffer(dest,
        fmt::format(java::main_end_template,
          "optional_average_field"_a = optional_average_field,
          "global_bias"_a = common::ToStringHighPrecision(node->global_bias),
          "main_footer"_a = main_tail_),
        indent);
  ...
  void HandleQNode(const QuantizerNode* node,
    ...
    AppendToBuffer(dest,
      fmt::format(java::quantize_loop_template,
