...
class Tree {
 ...
 private:
  ...
  template <typename X, typename Y>
  friend void BulkConstructTree(Tree<X, Y>& tree, int n_nodes,
      ...
      double const* impurity, std::int64_t total_sample_cnt, int n_targets,
      int max_num_class, bool is_classifier);
