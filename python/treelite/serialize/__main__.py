...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="CLI script to convert a model file to a "
        + "Treelite binary checkpoint"
    )
    parser.add_argument(
        "--input-model", type=str, required=True, help="Path to the tree model file"
    )
    parser.add_argument(
        "--input-model-type",
        type=str,
    ...
    if args.input_model_type == "sklearn_pkl":
        ...
        model = treelite.Model.load(
            args.input_model, model_format=args.input_model_type
        )
    else:
