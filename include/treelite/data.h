...
enum class DMatrixType : std::uint8_t {
  kDense = 0,
  kSparseCSR = 1
};
...
class DenseDMatrix : public DMatrix {
 ...
 public:
  ...
  template<typename ElementType>
  static std::unique_ptr<DenseDMatrix> Create(
      const void* data, const void* missing_value, std::size_t num_row, std::size_t num_col);
  static std::unique_ptr<DenseDMatrix> Create(
...
class CSRDMatrix : public DMatrix {
 ...
 public:
  ...
  template<typename ElementType>
  static std::unique_ptr<CSRDMatrix> Create(
      const void* data, const std::uint32_t* col_ind, const std::size_t* row_ptr,
      std::size_t num_row, std::size_t num_col);
  static std::unique_ptr<CSRDMatrix> Create(
      TypeInfo type, const void* data, const std::uint32_t* col_ind, const std::size_t* row_ptr,
      std::size_t num_row, std::size_t num_col);
  std::size_t GetNumRow() const override = 0;
  std::size_t GetNumCol() const override = 0;
  std::size_t GetNumElem() const override = 0;
