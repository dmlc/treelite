...
USER_OPTIONS = {
    "cmake-build-dir": UserOption(
        description="Build directory used for CMake build",
        value="build",
        is_boolean=False,
    ),
    "use-system-libtreelite": UserOption(
        description="Use libtreelite.so from system path", value=False, is_boolean=True
    ),
}
    ...
    def build_cmake_extension(self):
        ...
        libtreelite = os.path.abspath(
            os.path.join(
                CURRENT_DIR,
                os.path.pardir,
                USER_OPTIONS["cmake-build-dir"].value,
                lib_name(),
            )
        )
...
class InstallLib(install_lib.install_lib):
    ...
    def install(self):
        ...
        if USER_OPTIONS["use-system-libtreelite"].value:
            ...
            msg = (
                "use-system-libtreelite is specified, but "
                + lib_name()
                + " is not found in: "
                + lib_path
            )
        ...
        src_dir = os.path.join(
            CURRENT_DIR, os.path.pardir, USER_OPTIONS["cmake-build-dir"].value
        )
...
class Install(install.install):  # pylint: disable=too-many-instance-attributes
    ...
    user_options = install.install.user_options + list(
        (k + ("" if v.is_boolean else "="), None, v.description)
...
if __name__ == "__main__":
    ...
    with open(
        os.path.join(CURRENT_DIR, "treelite/VERSION"), "r", encoding="UTF-8"
    ) as f:
    ...
    setup(
        ...
        extras_require={"scikit-learn": ["scikit-learn"]},
        author="DMLC",
