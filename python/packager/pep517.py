"""
Custom build backend for Treelite Python package.
...
from .nativelib import locate_local_libtreelite, locate_or_build_libtreelite
...
def build_wheel(
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        pkg_path = workspace / "treelite"
        copytree_with_logging(TOPLEVEL_DIR / "treelite", pkg_path, logger=logger)
...
def build_sdist(
    ...
    logger = logging.getLogger("treelite.packager.build_sdist")

    if config_settings:
        raise NotImplementedError(
            "Treelite's custom build backend doesn't support config_settings option "
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        copytree_with_logging(
            TOPLEVEL_DIR / "treelite", workspace / "treelite", logger=logger
        )
        copytree_with_logging(
...
def build_editable(
    ...
    logger = logging.getLogger("treelite.packager.build_editable")
    ...
    if locate_local_libtreelite(TOPLEVEL_DIR, logger=logger) is None:
        raise RuntimeError(
            "To use the editable installation, first build libtreelite with CMake. "
            "See https://treelite.readthedocs.io/en/latest/install.html for detailed instructions."
        )
